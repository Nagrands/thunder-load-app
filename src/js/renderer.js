...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    window.electron.ipcRenderer.invoke("wg-get-config").then((cfg) => {
      console.log("[WG] autosend from config:", cfg.autosend); // ← отладка
      wgAutosendCheckbox.checked = !!cfg.autosend;
    });
    ...
    if (wgSendBtn) {
      wgSendBtn.addEventListener("click", () => {
        ...
        window.electron.ipcRenderer.invoke("wg-send-command")
          .then(() => {
            hideLater();
          })
          .catch(() => {
    ...
    if (kvnBtn) {
      kvnBtn.addEventListener("click", () => {
        ...
        requestAnimationFrame(() => {
          window.electron.ipcRenderer.invoke("kvn-command")
            .then(() => {
              hideLater();
            })
            .catch(() => {
