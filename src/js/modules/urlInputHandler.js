...
function initUrlInputHandler() {
  ...
  const syncIconPosition = () => {
    try {
      ...
      if (prog) {
        ...
        prog.style.borderRadius = getComputedStyle(wrapperEl).borderRadius || '12px';
      }
    } catch (_) {}
  };
  ...
  const renderPreview = (data) => {
    ...
    if (!data || !data.success) { card.style.display = 'none'; card.classList.remove('pos-top'); if (addAllBtn) addAllBtn.style.display='none'; return; }
    ...
    const sanitizeFilename = (s) => {
      try {
        const base = String(s || '').replace(/[\\/:*?"<>|]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    if (wrap && img && img.style.display !== 'none' && data.thumbnail) {
      if (!saveBtn) {
        ...
        saveBtn.innerHTML = '<i class="fa-solid fa-download" aria-hidden="true"></i>';
  ...
  const maybeFetchPreview = () => {
    ...
    if (!isValidUrl(url) || !isSupportedUrl(url)) { renderPreview(null); setPresetsVisible(false); return; }
  ...
  urlInput.addEventListener("focus", () => {
    ...
    try { if (urlInput.value && urlInput.value.length > 0) urlInput.select(); } catch(_) {}
  });
  ...
  window.addEventListener('resize', () => requestAnimationFrame(syncIconPosition));
}
