    ...
    <article class="${cardClass}">
      ...
      <section id="media-inspector-report" class="media-inspector-report hidden">
        ...
        <div class="media-inspector-streams-grid">
          <section
            id="media-inspector-warnings-section"
            class="media-inspector-stream-section media-inspector-stream-section--warnings media-inspector-warnings"
          >
          ...
          <section
            id="media-inspector-video-section"
            class="media-inspector-stream-section media-inspector-stream-section--video"
          >
            <div class="media-inspector-stream-section__header">
          ...
          <section
            id="media-inspector-audio-section"
            class="media-inspector-stream-section media-inspector-stream-section--audio"
          >
            <div class="media-inspector-stream-section__header">
          ...
          <section
            id="media-inspector-subtitle-section"
            class="media-inspector-stream-section media-inspector-stream-section--subtitle"
          >
            <div class="media-inspector-stream-section__header">
...
function createEmptyState(text, tone = "neutral") {
  const state = document.createElement("div");
  state.className = `media-inspector-empty-state media-inspector-empty-state--${tone}`;

  const badge = document.createElement("span");
  badge.className = "media-inspector-empty-state__badge";
  badge.textContent = "0";

  const body = document.createElement("p");
  body.className = "media-inspector-empty-state__text";
  body.textContent = text;

  state.append(badge, body);
  return state;
}
...
export function initMediaInspectorPanel({
  root,
  getEl = (id, node = document) => node.querySelector(`#${id}`),
  t,
  registerCleanup,
  initialFilePath = "",
  allowPickFile = true,
  ...
  if (!root.childNodes.length) {
    root.innerHTML = createMarkup(t, { allowPickFile, variant });
  }

  const pickFileBtn = getEl("media-inspector-pick-file", root);
  ...
  const stateTitleEl = getEl("media-inspector-state-title", root);
  const stateBodyEl = getEl("media-inspector-state-body", root);
  const stateMetaEl = getEl("media-inspector-state-meta", root);
  const reportEl = getEl("media-inspector-report", root);
  const copyFeedbackEl = getEl("media-inspector-copy-feedback", root);
  const warningsEl = getEl("media-inspector-warnings", root);
  const warningsCountEl = getEl("media-inspector-warnings-count", root);
  const warningsSectionEl = getEl("media-inspector-warnings-section", root);
  const videoListEl = getEl("media-inspector-video-streams", root);
  const audioListEl = getEl("media-inspector-audio-streams", root);
  const subtitleListEl = getEl("media-inspector-subtitle-streams", root);
  const videoCountEl = getEl("media-inspector-video-count", root);
  const audioCountEl = getEl("media-inspector-audio-count", root);
  const subtitleCountEl = getEl("media-inspector-subtitle-count", root);
  const videoSectionEl = getEl("media-inspector-video-section", root);
  const audioSectionEl = getEl("media-inspector-audio-section", root);
  const subtitleSectionEl = getEl("media-inspector-subtitle-section", root);
  const summaryContainerEl = getEl("media-inspector-summary-container", root);
  const summaryDurationEl = getEl("media-inspector-summary-duration", root);
  const summarySizeEl = getEl("media-inspector-summary-size", root);
  const summaryBitrateEl = getEl("media-inspector-summary-bitrate", root);
  const summaryScoreEl = getEl("media-inspector-summary-score", root);
  ...
  const setSectionState = (sectionEl, hasItems) => {
    if (!sectionEl) return;
    sectionEl.classList.toggle("is-empty", !hasItems);
    sectionEl.classList.toggle("is-populated", !!hasItems);
  };
  ...
  const renderEmptyLists = () => {
    ...
    setSectionState(warningsSectionEl, false);
    setSectionState(videoSectionEl, false);
    setSectionState(audioSectionEl, false);
    setSectionState(subtitleSectionEl, false);
  };
  ...
  const createStreamCard = (stream, type, index) => {
    ...
    const title = document.createElement("strong");
    title.textContent = t(`tools.mediaInspector.streamLabels.${type}`);
    const badge = document.createElement("span");
    badge.className = "media-inspector-stream-card__index";
  ...
  const renderReport = (report) => {
    ...
    const renderStreamGroup = (items, type, container, countEl, sectionEl) => {
      ...
      if (!items.length) {
        ...
        setSectionState(sectionEl, false);
        return;
      }
      items.forEach((item, index) => {
        container.appendChild(createStreamCard(item, type, index));
      });
      setSectionState(sectionEl, true);
    };
  ...
  if (initialFilePath) {
    void inspectFile(initialFilePath, { autoAnalyze: autoAnalyzeInitial });
  }
