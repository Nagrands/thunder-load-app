...
describe("toolsView quick actions", () => {
  ...
  test("renders the launcher inside the tools sidebar shell", async () => {
    ...
    expect(hero?.classList.contains("tab-hero")).toBe(true);
    expect(hero?.querySelector(".tab-hero__content")).not.toBeNull();
    expect(hero?.querySelector(".tab-hero__icon")).not.toBeNull();
    expect(hero?.querySelector(".tab-hero__art")).not.toBeNull();
    expect(hero?.querySelector(".title-content")).not.toBeNull();
    expect(heroArt?.getAttribute("aria-hidden")).toBe("true");
    expect(heroImage?.getAttribute("src")).toBe("../assets/img/tools-hero.png");
    ...
    expect(el.querySelector('[data-ui="tools-entrance-root"]')).not.toBeNull();
    expect(el.querySelectorAll('[data-ui="tools-launcher-card"]').length).toBe(
      8,
    );
    expect(
      el
