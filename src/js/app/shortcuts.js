...
function setupGlobalShortcuts(mainWindow) {
  ...
  const userShortcuts = store.get("userShortcuts", [
    { combo: isMac ? "Alt+1" : "Control+Shift+1", url: "https://www.youtube.com" },
    { combo: isMac ? "Alt+2" : "Control+Shift+2", url: "https://www.twitch.tv" },
    { combo: isMac ? "Alt+3" : "Control+Shift+3", url: "https://vkvideo.ru" },
    { combo: isMac ? "Alt+4" : "Control+Shift+4", url: "https://www.coub.com" },
  ]);

  try {
    const reloaded = globalShortcut.register(isMac ? "Command+R" : "Control+R", () => {
      if (mainWindow) mainWindow.reload();
    });

    if (reloaded) {
      log.info(`Global shortcut for reload registered (${isMac ? "Cmd+R" : "Ctrl+R"})`);
    } else {
