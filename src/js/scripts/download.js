// download.js (src/js/scripts/download.js)
...
/**
 * Парсит строку прогресса вида "[download] 45.3%"
 */
...
function selectFormatsByQuality(formats, desiredQuality) {
  ...
  if (desiredQuality === QUALITY_AUDIO_ONLY) {
    let audioFormats = formats.filter(
      (fmt) => fmt.acodec !== "none" && fmt.vcodec === "none",
    );
    let bestAudio = audioFormats.sort((a, b) => b.abr - a.abr)[0];
    // Если чистого аудио нет — fallback на muxed
    if (!bestAudio) {
      const muxed = formats.filter(
        (fmt) => fmt.acodec !== "none" && fmt.vcodec !== "none",
      );
      bestAudio = muxed.sort((a, b) => b.abr - a.abr || b.tbr - a.tbr)[0];
      if (!bestAudio) throw new Error("No audio or muxed formats found");
      return {
        videoFormat: bestAudio.format_id, // используется muxed как источник
        audioFormat: null,
        audioExt: bestAudio.ext,
        resolution: "audio (muxed)",
        fps: bestAudio.fps || null,
      };
    }
    return {
      videoFormat: null,
      ...
      audioExt: bestAudio.ext,
      resolution: bestAudio.ext,
      fps: null,
    };
  }

  if (desiredQuality === QUALITY_SOURCE) {
    // Сначала пытаемся найти разделенные форматы
    const videoFormats = formats.filter(
      (fmt) => fmt.vcodec !== "none" && fmt.acodec === "none",
    );
    const audioFormats = formats.filter(
      (fmt) => fmt.acodec !== "none" && fmt.vcodec === "none",
    );
    if (videoFormats.length > 0 && audioFormats.length > 0) {
      videoFormat = videoFormats.sort(
        (a, b) => b.height - a.height || b.tbr - a.tbr,
      )[0];
      audioFormat = audioFormats.sort((a, b) => b.abr - a.abr)[0];
      if (!videoFormat || !audioFormat)
        throw new Error("Suitable formats for source quality not found.");
      return {
        videoFormat: videoFormat.format_id,
        audioFormat: audioFormat.format_id,
        resolution:
      ...
      // fallback: ищем muxed формат
      const muxed = formats.filter(
        (fmt) => fmt.vcodec !== "none" && fmt.acodec !== "none",
      );
      const bestMuxed = muxed.sort(
        (a, b) => b.height - a.height || b.tbr - a.tbr,
      )[0];
      if (!bestMuxed)
        throw new Error("No suitable muxed format found for source quality.");
      return {
        videoFormat: bestMuxed.format_id,
        audioFormat: null,
        audioExt: bestMuxed.ext,
        resolution:
          bestMuxed.width && bestMuxed.height
            ? `${bestMuxed.width}x${bestMuxed.height}`
            : "unknown",
        fps: bestMuxed.fps || null,
      };
    }
  } else {
    // Для остальных качеств (FHD, HD, SD)
    const qualityMap = {
      [QUALITY_FHD]: 1080,
      [QUALITY_HD]: 720,
      [QUALITY_SD]: 360,
    };
    const desiredHeight = qualityMap[desiredQuality];
    if (!desiredHeight) throw new Error(`Invalid quality: ${desiredQuality}`);
    let videoFormats = formats.filter(
      (fmt) =>
        fmt.vcodec !== "none" &&
        fmt.acodec === "none" &&
        fmt.height === desiredHeight,
    );
    if (videoFormats.length === 0) {
      // fallback: ищем muxed формат с высотой не больше desiredHeight
      const muxed = formats.filter(
        (fmt) =>
          fmt.vcodec !== "none" &&
          fmt.acodec !== "none" &&
      ...
      audioFormat = audioFormats.sort((a, b) => b.abr - a.abr)[0];
      if (!audioFormat) throw new Error("Audio format not found");
    }
    videoFormat = videoFormats.sort((a, b) => b.tbr - a.tbr)[0];
    const resolution =
      videoFormat.width && videoFormat.height
        ? `${videoFormat.width}x${videoFormat.height}`
        : "unknown";
    return {
      videoFormat: videoFormat.format_id,
      audioFormat: audioFormat ? audioFormat.format_id : null,
      resolution,
      fps: videoFormat.fps || null,
    };
  }
...
async function downloadMedia(
  ...
  try {
    ...
    const audioOutputTemp = path.join(downloadPath, `audio_${uniqueId}.webm`);
    // Если используется muxed fallback (audioFormat не валиден), итоговый файл будет переименован.
    ...
    if (
      ...
      // Переименовываем скачанный видеофайл с корректным именем
      const properOutput = path.join(downloadPath, `${sanitizedFilename}.mp4`);
      try {
