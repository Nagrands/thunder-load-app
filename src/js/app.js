...
const { ensureAllDependencies } = require("./scripts/download.js");
...
if (!app.requestSingleInstanceLock()) {
  ...
  async function restoreDownloadPath() {
    try {
      const savedPath = await mainWindow.webContents.executeJavaScript(
        `window.localStorage.getItem('downloadPath')`,
      );
      if (typeof savedPath === "string" && savedPath.trim() !== "") {
        downloadPath = savedPath;
        dependencies.downloadState.downloadPath = downloadPath; // Обновляем downloadState
        store.set("downloadPath", downloadPath); // Сохраняем путь в store
      }
    } catch (error) {
