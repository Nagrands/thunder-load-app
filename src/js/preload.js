...
try {
  ...
  const VALID_CHANNELS = Object.freeze([
    ...
    "toggle-auto-launch",
    "tray-state-update",
    "windows-tray-menu:get-state",
    "windows-tray-menu:action",
    "windows-tray-menu:close",
    "tools:checkUpdates",
   ...
   *   runDependencyAction: (payload: { id: "ytDlp"|"ffmpeg"|"deno", action: "install"|"update"|"reinstall" }) => Promise<{ success: boolean, toolId?: string, version?: string, error?: string }>,
  ...
  const electronAPI = {
    ...
    tools: {
      ...
      runDependencyAction: (payload) =>
        safeInvoke("tools:runDependencyAction", payload),
