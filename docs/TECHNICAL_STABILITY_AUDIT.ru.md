...
- Windows packaged smoke выполнен на свежей `win-unpacked` сборке от текущего HEAD. При активном Web Control preview packaged-owned `yt-dlp` был прямым потомком main process и завершился при штатном закрытии окна; после shutdown не осталось Thunder/yt-dlp/FFmpeg/ffprobe процессов.
- Отдельный Windows Player smoke выбрал вторую EAC3-аудиодорожку локального multitrack MKV и подтвердил долгоживущий HLS FFmpeg. Процесс FFmpeg работал 70,130 с, при quit получил `SIGTERM`; coordinator cleanup занял 54 мс, полное исчезновение дерева процессов — 237 мс.
- Windows recreate проверен в одном packaged application process через локальный smoke inspector: штатный Windows `window-all-closed → quit` listener был временно снят только внутри тестового процесса, окно `id=1` уничтожено, `activate` создал рабочее окно `id=2` в том же PID. Новый renderer загрузился, Web Control повторно запустился и отвечал на status API; финальный coordinator cleanup занял 7 мс, полное исчезновение дерева процессов — 214 мс.
...
- После Windows smoke исходный `config.json` восстановлен побайтно; тестовый media-файл не изменялся, оставшихся Thunder/yt-dlp/FFmpeg/ffprobe процессов нет.
