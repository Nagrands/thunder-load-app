...
|---|---|
| Added a background video preview to the Downloader tab for YouTube links | Pasting a YouTube URL can now start a muted looping video behind the Downloader UI; the background now switches more smoothly, pauses while the window is hidden, and tries to continue from the same moment when a preview source is refreshed, while every other source and any unplayable YouTube case still falls back to the existing static preview |
